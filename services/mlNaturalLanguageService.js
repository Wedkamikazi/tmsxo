...
exports.mlNaturalLanguageService = void 0;
...
class MLNaturalLanguageService {
    ...
    async buildVocabularyFromTransactions() {
    ...
    async analyzeTransaction(transaction) {
        ...
        try {
            ...
            const [entities, sentiment, intent, topics, complexity] = await Promise.all([
    ...
    async analyzeSentiment(inputTensor) {
    ...
    async classifyIntent(inputTensor) {
        if (!this.intentClassificationModel) {
            return { type: 'unknown', confidence: 0.5 };
        }
        const prediction = this.intentClassificationModel.predict(inputTensor);
        const probabilities = await prediction.data();
        let maxIndex = 0;
        let maxProb = probabilities[0];
        for (let i = 1; i < probabilities.length; i++) {
        ...
        const intentTypes = ['payment', 'transfer', 'deposit', 'withdrawal', 'fee', 'refund'];
    ...
    // TOPIC EXTRACTION
    async extractTopics(inputTensor) {
        if (!this.topicModelingModel) {
            return [];
        }
        const prediction = this.topicModelingModel.predict(inputTensor);
    ...
    generateSuggestions(description, entities, sentiment, intent) {
        const suggestions = {
            missingInfo: []
        };
    ...
    generateBetterDescription(originalDescription, entities, intent) {
    ...
    suggestCategory(intent, entities) {
    ...
    async calculateSemanticSimilarity(transaction1, transaction2) {
    ...
    async getTransactionEmbedding(description) {
        if (!this.embeddingModel) {
    ...
    classifyPatternCategory(pattern) {
        if (pattern.includes('payment') || pattern.includes('pay'))
            return 'payment';
        if (pattern.includes('transfer') || pattern.includes('wire'))
            return 'transfer';
        if (pattern.includes('deposit'))
            return 'deposit';
        if (pattern.includes('withdrawal') || pattern.includes('atm'))
            return 'withdrawal';
        if (pattern.includes('fee') || pattern.includes('charge'))
            return 'fee';
        if (pattern.includes('refund') || pattern.includes('return'))
            return 'refund';
        return 'general';
    }
    // UTILITY METHODS
    prepareModelInput(description) {
    ...
    calculateBasicSimilarity(transaction1, transaction2) {
        const structural = this.calculateStructuralSimilarity(transaction1.description, transaction2.description);
        const temporal = this.calculateTemporalSimilarity(transaction1.date, transaction2.date);
        const amount = this.calculateAmountSimilarity(transaction1, transaction2);
    ...
    calculateStructuralSimilarity(desc1, desc2) {
    ...
    calculateTemporalSimilarity(date1, date2) {
    ...
    calculateAmountSimilarity(transaction1, transaction2) {
    ...
    generateSimilarityExplanation(semantic, structural, temporal, amount) {
        const factors = [];
        if (semantic > 0.8)
            factors.push('very similar meaning');
        else if (semantic > 0.6)
            factors.push('similar meaning');
        else if (semantic > 0.4)
            factors.push('somewhat similar meaning');
        if (structural > 0.7)
            factors.push('similar structure');
        if (temporal > 0.8)
            factors.push('close in time');
        if (amount > 0.9)
            factors.push('similar amounts');
        return factors.length > 0 ? `Similar due to: ${factors.join(', ')}` : 'Low similarity across all factors';
    }
    getFallbackAnalysis(_description) {
        return {
            entities: {
                merchants: [],
                amounts: [],
    ...
    dispose() {
        ...
        if (this.topicModelingModel)
            this.topicModelingModel.dispose();
        this.vocabulary.clear();
...
exports.mlNaturalLanguageService = new MLNaturalLanguageService();
